...
4. **TIMED_WAITING** 时间等待 thread.sleep   object.wait  thread.join  **时间参数**
...
- CountDownLatch/Join/Future.get = Executors.submit().get()

- Kotlin coroutine 独有 async()
...
1. **与线程强相关、线程内部私有、但在多个方法间需要共享的数据**,可以用来**存储线程上下文信息**

2. `ThreadLocal.ThreadLocalMap threadLocals`  

3. ThreadLocalMap数据结构，**Entry动态数组**

   **使用 try-finally 确保及时清理 ThreadLocal 变量**

4. **在 set() 或 get() 过程中进行“惰性清理”，遍历发现 Key 为 null 的位置时，会直接replace掉该Entry**
