...
1. ThredLocal **线程的局部变量**，在每个线程中都是独立的，为每个线程保存一个自己的变量，
2. ThreadLocal本身并不存储变量，变量存储在线程自己的实例变量`ThreadLocal.ThreadLocalMap`中
3. 一般是static final,可以在多个函数共享同一个变量
4. 线程安全：每个线程都只会访问自己ThreadLocalMap 保存的变量，所以不存在线程安全问题
5. 记住要回收
