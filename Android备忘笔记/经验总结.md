...
|  short/char  | 2 byte |   0    | -2^15 - 2^15-1           |
...
->`ActivityThread` 线程会调用 `Instrumentation` 类中的 `newActivity()` 方法创建一个新的 Activity 实例，并通过反射机制调用其 `onCreate()` ，关联contentView与decorView
...
class ViewRootImpl{
	...
	//移除屏障 加绘制
  	void  doTraversal(){
       移除同步屏障()
     performTraversals（）//绘制
    } 
	
	void performTraversals(){
        final View host = mView;
		host.dispatchAttachedToWindow()  //view post到runQueue的延迟消息被激活
        performMessure()
		performLayout()
		performDraw()    
    }          
