...
启动耗时：A界面（maybeLauncher） start B界面

一阶段： 从startBActivity（）到 A pause（） 

```java
ActivityA.startActivity->
ActivityManagerNative.getDefault.startActivity->通知ActivityThread，pauseA
ActivityThread.startPausingLocked->
